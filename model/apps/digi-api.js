...
// __________Count Pages______________
/* async function countPages (){
  const res =await listOfVariants()
  console.log('RES:::',res)
  // console.log('pages:',countPages().totalPages);
}
console.log('countPages :',countPages()); */

// console.log('---', listOfVariants(1).then(res=>{console.log(res);}));

// console.log('###', async ()=> await listOfVariants(1));

//______________OK____OK_______________
async function fetchDataFromDigikalaAPI() {
  try {
    const productPerPage = 99;
    await listOfVariants(1, 2);
    // console.log("totalProducts : ", Number(totalProducts)+1); //ok
    const totalPages = totalProducts / productPerPage;
    // console.log('totalPages:',Number(totalPages).tointeger()+1);
    // console.log('totalPages:',parseInt(totalPages)+1); // ok
    console.log("cmd2024" + (parseInt(totalPages) + 1) + 1594); // ok
    // for (let i = 1; i <=  1; i++) {
    for (let i = 1; i <= totalPages + 1; i++) {
      // for (let i = 1; i <= totalProducts / productPerPage + 1; i++) {
      // console.log("page :", i); //ok
      i < 10 ? console.log(crypto.randomUUID() +Math.floor(Math.random()*10)+  i + "-" + crypto.randomUUID()) : console.log(crypto.randomUUID() + i+"-"+crypto.randomUUID())
      // i < 10 ? console.log(crypto.randomUUID() +Math.floor(Math.random()*10)+  i ) : console.log(crypto.randomUUID() + i)
    ...
    if (errPage.length === 0) {
      ...
      behzad.writeJson(
        path.join(
          __dirname,
          "../data/list-products-from-digi-api/product-list.json"
        ),
        allProducts
      );
      allProducts = [];
