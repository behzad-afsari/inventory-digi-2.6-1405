...
function priceHandle(){
   ...
   productRow.forEach(item=>{
      const itemCount = item.querySelector("#orderCount")
      const itemPrice = item.querySelector('#orderPrice')
      // console.log(itemCount,itemPrice);
      itemCount.addEventListener('input',(e)=>{
         itemPrice.value = item.querySelector('#lastBuyPrice').textContent
         // console.log(item.querySelector('#lastBuyPrice').textContent);
      })
...
function getData(event) {
   ...
   productRow.forEach((item) => {
      // console.log(item);
      if (item.querySelector("#orderCount").value) {
         const obj = {};
         ...
         obj["فی"] = parseFloat(item.querySelector("#orderPrice").value);
         // withoutPriceIsOk = true
         if (!obj["فی"]) {
            __price = prompt(obj["نام کالا"] + "قیمت ندارد");
            obj["فی"] = __price
            const x = item.querySelector("#orderPrice");
            console.log(__price);
            x.value = obj["فی"];
         }
         // console.log(obj);
